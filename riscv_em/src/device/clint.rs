...
impl Clint {
    ...
    pub fn read(&mut self, addr: u32) -> u32 {
        ...
        match addr {
            0x200bffc => self.mtimeh,
            0x200bff8 => self.mtime,
            0x2004004 => self.mtimecmph,
            0x2004000 => self.mtimecmp,
            _ => 0,
        }
    }
    pub fn write(&mut self, addr: u32, data: u32) {
        let addr = addr as usize - self.base;
        match addr {
            0x200bffc => self.mtimeh = data,
            0x200bff8 => self.mtime = data,
            0x2004004 => self.mtimecmph = data,
            0x2004000 => self.mtimecmp = data,
            _ => {}
        };
    }
}
